...
export const AdminDashboard: React.FC = () => {
  ...
  /* --- Helper for Trends --- */
  const getTrend = (data: any[], daysArg: number = 7) => {
    const now = Date.now();
    const ms = daysArg * 24 * 60 * 60 * 1000;
    const recent = data.filter(item => (item.createdAt || 0) > now - ms).length;
    return recent;
  };
  ...
  const getDailyTrend = (data: any[]) => {
    ...
    return data.filter(item => (item.createdAt || 0) >= startOfDay).length;
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const newTeamsWeek = getTrend(t, 7);
    ...
    <div className="text-white font-sans selection:bg-[#00ff9d] selection:text-black">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        <StatCard
          ...
          trendUp={stats.usersTrend !== "Stable"}
          delay={0.1}
        />
        <StatCard
          icon={Target}
          ...
          trendUp={stats.playersTrend !== "No new players"}
          delay={0.2}
        />
        <StatCard
          icon={Shield}
          ...
          value={stats.teams}
          trend={stats.teamsTrend}
          trendUp={stats.teamsTrend !== "Stable"}
          delay={0.3}
        />
        <StatCard
          icon={Activity}
          ...
          trendUp={stats.matchesTrend !== "Low activity"}
          delay={0.4}
        />
      </div>
